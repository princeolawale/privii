...
export function TagProfileClient({ record }: { record: PriviiTagRecord }) {
  ...
  return (
    <div className="mx-auto w-full max-w-xl pt-10 sm:pt-14">
      <div className="relative rounded-[34px] border border-border bg-card/95 px-6 pb-8 pt-20 shadow-[0_30px_120px_rgba(0,0,0,0.5)] sm:px-8 sm:pb-10 sm:pt-24">
        <div className="space-y-9">
          ...
          {!wallet.connected ? (
            <p className="text-sm text-secondary">
              Connect a wallet from the header to continue.
            </p>
          ) : null}

          {error ? <p className="text-sm text-red-400">{error}</p> : null}

          <Button className="mt-2 w-full" disabled={!canPay || isPaying} onClick={handlePay}>
            {isPaying ? (
